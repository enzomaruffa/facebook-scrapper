...
# Writing results
for page in pages:
	f = open(str(page.page_id)+".csv", "w+", encoding="utf-8")
	f.write('page_id, post_id, message, permalink_url, reactions_count, comment_count, share_count \n')
	for post in page.posts:
		f.write(str(page.page_id) + ',' + str(post.post_id) + ',' + str(post.message) + ',' + str(post.permalink_url) + ',' + str(post.reactions_count) + ','  + str(post.comment_count) + ',' + str(post.share_count) + '\n')
